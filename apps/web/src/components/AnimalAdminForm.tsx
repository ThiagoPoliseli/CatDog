...
export function AnimalAdminForm({ action, animals, breeds, sizes, species }: Props) {
  ...
  const [previewUrl, setPreviewUrl] = useState<string | null>(defaults?.imageUrl ?? null);
  ...
  return (
    <form action={action} className="form-grid">
      ...
      {defaults?.imageUrl ? (
        <input name="existingImageUrl" type="hidden" value={defaults.imageUrl} />
      ) : null}
      ...
      <div className="grid gap-1.5">
        ...
        {previewUrl && (
          <div style={{ position: "relative", width: 120, height: 90, borderRadius: 6, overflow: "hidden", marginBottom: 6 }}>
            <Image
        ...
        <p className="text-xs text-muted-foreground">
          JPG, PNG ou WebP · max 5 MB{defaults ? " · deixe vazio para manter a imagem atual" : ""}
        </p>
      </div>
