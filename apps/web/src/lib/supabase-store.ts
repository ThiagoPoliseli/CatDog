...
export async function updateSupabaseAdoptionRequestStatus(
  ...
  await assertNoError(
    (await supabase.from("adoption_requests").update({ status }).eq("id", id)).error,
  );
  ...
  if (status === "approved" || status === "completed") {
    await assertNoError(
      (await supabase.from("animals").update({ status: "adopted" }).eq("id", request.animal_id)).error,
    );
  } else if (status === "rejected" || status === "cancelled") {
    const { data: activeRequests } = await supabase
      .from("adoption_requests")
      .select("id")
      .eq("animal_id", request.animal_id)
      .in("status", ["received", "reviewing", "approved"]);

    if (!activeRequests || activeRequests.length === 0) {
      await assertNoError(
        (await supabase.from("animals").update({ status: "available" }).eq("id", request.animal_id)).error,
      );
    }
  }
}
